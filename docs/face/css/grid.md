...
- fr：片段，为了方便表示比例关系
    ...
    > `grid-template-columns: 1fr 2fr 1fr` 表示第一列和第三列宽度相同，第二列宽度是第一列和第三列的两倍
...
若为`column`，则代表"先列后行"，即先填满第一列，再开始放入第二列

### justify-items & align-items & place-items

`justify-items` 属性设置 **单元格** 内容的水平位置（左中右），`align-items` 属性设置 **单元格** 的垂直位置（上中下）
...
意思为将1号项目位于e区域，与上述讲到的 `grid-template-areas` 搭配使用
